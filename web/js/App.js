var App = function(aSettings, aCanvas) {
	...
	var model, canvas, context, webSocket, webSocketService, mouse = {
		x : 0,
		y : 0,
		worldx : 0,
		worldy : 0,
	...
	app.update = function() {
		...
		if (keyNav.x != 0 || keyNav.y != 0) {
			model.userTadpole.userUpdate(model.tadpoles, model.userTadpole.x
					+ keyNav.x, model.userTadpole.y + keyNav.y);
		} else {
			var mvp = getMouseWorldPosition();
			mouse.worldx = mvp.x;
			mouse.worldy = mvp.y;
			model.userTadpole.userUpdate(model.tadpoles, mouse.worldx,
					mouse.worldy);
		}
	...
	app.onSocketOpen = function(e) {
		...
		if (uri.queryKey.oauth_token) {
			app
					.authorize(uri.queryKey.oauth_token,
							uri.queryKey.oauth_verifier)
		}
	...
	var getMouseWorldPosition = function() {
		return {
			x : (mouse.x + (model.camera.x * model.camera.zoom - canvas.width / 2))
					/ model.camera.zoom,
			y : (mouse.y + (model.camera.y * model.camera.zoom - canvas.height / 2))
					/ model.camera.zoom
		}
	}
	...
	(function() {
		...
		model.camera = new Camera(canvas, context, model.userTadpole.x,
				model.userTadpole.y);
